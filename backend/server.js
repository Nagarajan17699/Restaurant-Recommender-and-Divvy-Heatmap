...
router.route('/places/find_with_zip').post((req, res) => {

  var str = JSON.stringify(req.body, null, 4);


  find_places_with_zipcode_from_yelp(req.body.find, req.body.where).then(function (response) {
      var hits = response;
      res.json(places_found);
  });
...
router.route('/stations/fetch_all_divvy_stations_data_custom_data').post((req, res) => {

  get_all_divvy_stations_log_custom_data(req.body.timeRange,req.body.newTimeRangeSelection).then(function (response) {
      var hits = response;
...
async function find_places_with_zipcode_from_yelp(place, where) {

  console.log("find places called ");
  places_found = [];
...
async function get_all_divvy_stations_log_custom_data(timeRange, newTimeRangeSelection) {
  var start_datetime_var = new Date();
  var scrollVal;
  ...
  all_docks_found = [];
    ...
    if(!isBeginningOfTimeRangeSet){
      ...
      var start_datetime_var = new Date();
      var start_datetime_var_2 = new Date();
      var targetTime = new Date( timeRange );
      var tzDifference = targetTime.getTimezoneOffset();
  ...
  results.hits.hits.forEach(function (hit) {
    allRecords.push(hit);
    if((((hit._source.totalDocks - hit._source.availableDocks)/hit._source.totalDocks)*100) > 80 ) {
    var docks = {
                "availableDocks": hit._source.availableDocks,
                "latitude": hit._source.latitude,
                "longitude": hit._source.longitude
